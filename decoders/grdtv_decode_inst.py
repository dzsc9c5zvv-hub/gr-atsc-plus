#!/usr/bin/env python3
"""Diagnostic decoder: same as v5 but with instrumented FS checker.
Stderr will print PN511/PN63 histograms + field-sync counters per ~50k segments.
...
class atsc_inst_tb(gr.top_block):
    def __init__(self):
        gr.top_block.__init__(self, "atsc_inst")
...
tb.start()
tb.wait()
print(f"inst decode done: {IN} -> {OUT}", file=sys.stderr)
